...
class MissionMemory:
    ...
    def __init__(self):
        ...
        # RETURN_HOME/LANDING. Captured exactly once by mission.py, only
        # after the vehicle has stabilized at TAKEOFF_ALTITUDE (see
        # _state_takeoff) -- not on the ground, not mid-climb. save_start_position
        # is a no-op once set, so nothing downstream can ever overwrite it.
        # KURSAD40 root-cause fix: this replaces the earlier home_position
        # (captured on the ground during INITIALIZATION, before OFFBOARD/
        # takeoff even started) and a short-lived finish_line_position
        # (sampled at the instant altitude first crossed 3m mid-climb, not
        # a settled position) -- two competing, ambiguous sources of "the
        # return point" instead of one.
    ...
    def save_start_position(self, gps, ned, alt, heading):
        ...
        print("[MISSION] ==============================")
        print("[MISSION] MISSION START POSITION SAVED")
        print(f"[MISSION]   N:   {data['north']:.3f}")
        print(f"[MISSION]   E:   {data['east']:.3f}")
        print(f"[MISSION]   ALT: {data['alt']:.3f}")
        print(f"[MISSION]   YAW: {data['heading']:.1f}")
        print("[MISSION] ==============================")
