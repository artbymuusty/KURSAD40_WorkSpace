...
class DebugView:
    ...
    def draw_dashboard(self, snapshot: UISnapshot, events: list, path_history: list = None):
        ...
        self._text(dashboard, f"NED: {snapshot.ned[0]:.1f}, {snapshot.ned[1]:.1f}, {snapshot.ned[2]:.1f}",
                    x0 + 10, body_y + 44, self.COL_TEXT_DIM, 0.53)
        ...
        # Auto-fits to every known mission point (Home, vehicle, Drop1,
        # Drop2) with a single equal-aspect scale for both axes -- the
        # previous version used a fixed +/-20m radius and, in the ALT_GZ
        # variant, independent N/E scales that stretched the map out of
        # real-world proportion. A shared scale means distances and marker
        # positions on screen actually match the real geometry between
        # Home/Drop1/Drop2/the vehicle.
        panel_h = heights["MISSION MAP"]
        vn, ve, _ = snapshot.ned
        ...
        # origin) are known -- matches the old fixed view's rough scale.
        MIN_SPAN = 40.0
        ...
        # Return-to-start line: only drawn once RETURN_HOME/LANDING has
        # actually begun, from the vehicle's current position straight to
        # the recorded start position -- makes the commanded maneuver visible,
        # not just its endpoint.
        if snapshot.mission_state in ("RETURN_HOME", "LANDING") and snapshot.start_ned:
        ...
        # the actual RETURN_HOME/LANDING target. Drawn dim/gray with a "?" until
        ...
        # instead of only a schematic crosshair.
...
class UIWorker:
    ...
    def __init__(self):
        ...
        self.debug_view.window_name = "Mission Dashboard"
    ...
    def _run(self):
        while self.running:
            try:
                ...
                t0 = time.time()
                self.debug_view.draw_dashboard(snapshot, self.events, self.path_history)
